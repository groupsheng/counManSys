...
conf.guquanCooperation.url = {
	...
	deleteObj : '/houtai/guquanCooperation/deleteObj'
};
conf.guquanCooperation.$pageDom = null;
conf.guquanCooperation.$datagrid = null;
conf.guquanCooperation.$form = null;
conf.guquanCooperation.editor = null;
conf.guquanCooperation.init = function() {
	...
	conf.guquanCooperation.$datagrid = $this.$pageDom
			.find('#guquanCooperation-page-table');
	$this.$form = $('#guquanCooperation_edit_dlg').find('form');
};
conf.guquanCooperation._bindClick = function() {
	var $this = this;
	$this.editor = KindEditor.create(
			'textarea[name="guquanCooperation.main_text"]', {
				urlType : 'absolute',
	...
	$('#guquanCooperation_edit_dlg').find('#guquanCooperation-edit-save')
			.click(
					function() {
						$.messager.progress({
							msg : '加载中...'
						...
						$this.editor.sync();
						var data = $('#guquanCooperation_edit_dlg')
								.find('form').serialize();
						$.ajax({
							type : "POST",
							url : $this.url.save,
							...
							success : function(msg) {
								if (msg.type == 'success') {
									...
									$('#guquanCooperation_edit_dlg').dialog(
											"close");
								} else {
	...
	$('#guquanCooperation_edit_dlg').find('#guquanCooperation-edit-cancel')
			.click(function() {
...
conf.guquanCooperation.search = function() {
	this.$datagrid.datagrid('load', {
		title : $('#search-guquanCooperation-page input[name="title"]').eq(0)
				.val(),
	});
};
...
conf.guquanCooperation.deleteObj = function(id) {
	var $this = this;
	$.messager.progress({
		msg : '加载中...'
	});
...
conf.guquanCooperation.format = function(val, row) {
	return '<a href="javascript:conf.guquanCooperation.edit(\''
			+ row.id
			+ '\');">编辑</a><span style=\"margin-left:5px;margin-right:5px;\">|</span><a href="javascript:conf.guquanCooperation.deleteObj(\''
			+ row.id + '\');">删除</a>';
};